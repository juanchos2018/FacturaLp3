...
//        }
        ...
        int pos = misDatos.posicionProducto(((Opcion)cboProducto.getSelectedItem()).getValor());
         ...
         //JOptionPane.showMessageDialog(rootPane,String.valueOf(pos));
...
//        }
        ...
        int numFac = misDatos.getNumPedido() + 1;
        misDatos.setNumPedido(numFac);
        //grabamos la factura
        FileWriter fw = null;
        PrintWriter pw = null;
        FileWriter fwd = null;
        ...
        try {
            fw = new FileWriter("Data/Pedido.txt", true);
            pw = new PrintWriter(fw);
            
            fwd = new FileWriter("Data/DetallePedido.txt", true);
            pwd = new PrintWriter(fwd);
            //encabezado de factura
            String aux =  numFac + "|" 
                    + ((Opcion)cboCliente.getSelectedItem()).getValor() + "|"
                    + ((Opcion)cboCliente.getSelectedItem()).getDescripcion() + "|"
                    + txtTotalValor.getText()+"|"
                    + txtFecha.getText();
            pw.println(aux);
            
            //detalle de Pedido we 
            int num = tblFactura.getRowCount();          
            for (int i = 0; i < num; i++) {
             String aux1 =numFac+"|"
        ...
        JOptionPane.showMessageDialog(rootPane, "Pedido " + numFac + " generada con éxito");
